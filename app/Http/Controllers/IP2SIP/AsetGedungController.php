...
class AsetGedungController extends Controller
{
    public function index(Request $request) {
        $aset_gedung = new AsetGedung();
        if ($request->bsip_id) {
            $ip2sip_id = mIP2SIP::where('bsip_id', $request->bsip_id)->distinct()->pluck('id');
            $$aset_gedung = $aset_gedung->whereIn('ip2sip_id', $ip2sip_id);
        }
        $aset_gedung = $aset_gedung->paginate(10);
    ...
    public function store(Request $request) {
        ...
        if (!$aset_gedung) return back()->withErrors('failed to store data');
        return redirect()->route('aset.gedung')->with('success', 'created');
    }

    public function update($id, Request $request) {
        $aset_gedung = AsetGedung::find(Crypt::decryptString($id));
        if (!$aset_gedung) return back()->withErrors('data not found');
        $request->validate([
            'ip2sip_id' => 'required|numeric',
            'jenis_aset' => 'required|string',
            'luas_lahan' => 'required|numeric',
        ...
        $aset_gedung = $aset_gedung->update([
            'ip2sip_id' => $request->ip2sip_id,
            'jenis_aset' => $request->jenis_aset,
            'luas_lahan' => $request->luas_lahan,
            'tahun_perolehan' => $request->tahun_perolehan,
        ...
        if (!$aset_gedung) return back()->withErrors('failed to update data');
        return redirect()->route('aset.gedung')->with('success', 'updated');
    }
    ...
    public function destroy($id) {
        ...
        if (!$aset_gedung) return back()->withErrors('data not found');
        $aset_gedung->delete();
        if ($aset_gedung) return back()->withErrors('failed to delete data');
        return redirect()->route('aset.gedung')->with('success', 'deleted');
    }
}
