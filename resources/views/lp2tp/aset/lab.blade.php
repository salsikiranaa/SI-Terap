...
<section class="asset-dashboard">
    <div class="container">
        <h2 class="page-title">Aset - Laboratorium</h2>
        <p class="subtitle">Pendataan Sarana dan Prasarana Berupa Laboratorium</p>
        
        <div class="content-card">
            <div class="filter-section">
                <form action="{{ route('aset.lab') }}" method="GET" class="filter-container">
                    ...
                    <tbody>
                        @foreach ($aset_lab as $al)
                            <tr>
                                ...
                                <td>{{ $al->jenis_aset }}</td>
                                <td class="text-end">{{ number_format($al->luas_lahan, 0, ',', '.') }}</td>
                                <td class="text-center">{{ $al->tahun_perolehan }}</td>
            ...
            <div class="pagination-container">
                <div class="pagination">
                    <a href="{{ route('aset.lab', [...request()->query(), 'page' => $aset_lab->currentPage()-1]) }}" class="page-item {{ $aset_lab->currentPage() == 1 ? 'disabled' : '' }}">&lt;</a>
                    
                    @if ($aset_lab->lastPage() > 1)
                        @php
                            $startPage = max(1, $aset_lab->currentPage() - 2);
                            $endPage = min($aset_lab->lastPage(), $aset_lab->currentPage() + 2);
                            
                            if ($startPage > 1) {
                                echo '<a href="'.route('aset.lab', [...request()->query(), 'page' => 1]).'" class="page-item">1</a>';
                                if ($startPage > 2) {
                            ...
                            for ($i = $startPage; $i <= $endPage; $i++) {
                                if ($i == $aset_lab->currentPage()) {
                                    ...
                                    echo '<a href="'.route('aset.lab', [...request()->query(), 'page' => $i]).'" class="page-item">'.$i.'</a>';
                                }
                            }
                            
                            if ($endPage < $aset_lab->lastPage()) {
                                if ($endPage < $aset_lab->lastPage() - 1) {
                                    echo '<span class="dots">...</span>';
                                }
                                echo '<a href="'.route('aset.lab', [...request()->query(), 'page' => $aset_lab->lastPage()]).'" class="page-item">'.$aset_lab->lastPage().'</a>';
                            }
                        @endphp
                    @endif
                    
                    <a href="{{ route('aset.lab', [...request()->query(), 'page' => $aset_lab->currentPage()+1]) }}" class="page-item {{ $aset_lab->currentPage() == $aset_lab->lastPage() ? 'disabled' : '' }}">&gt;</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function exportTableToExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Aset Laboratorium");
        
        // Membuat nama file dengan tanggal saat ini
        var now = new Date();
        var fileName = 'Aset_Laboratorium_' + 
                       now.getFullYear() + 
