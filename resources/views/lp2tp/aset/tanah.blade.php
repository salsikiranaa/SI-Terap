...
<section class="asset-dashboard">
    <div class="container">
        <h2 class="page-title">Aset - Tanah</h2>
        <p class="subtitle">Pendataan Sarana dan Prasarana Berupa Tanah</p>
        
        <div class="content-card">
            <div class="filter-section">
                <form action="{{ route('aset.tanah') }}" method="GET" class="filter-container">
                    ...
                    <tbody>
                        @foreach ($aset_tanah as $at)
                            <tr>
                                ...
                                <td>{{ $at->jenis_aset }}</td>
                                <td class="text-end">{{ number_format($at->luas_lahan, 0, ',', '.') }}</td>
                                <td class="text-center">{{ $at->tahun_perolehan }}</td>
            ...
            <div class="pagination-container">
                <div class="pagination">
                    <a href="{{ route('aset.tanah', [...request()->query(), 'page' => $aset_tanah->currentPage()-1]) }}" class="page-item {{ $aset_tanah->currentPage() == 1 ? 'disabled' : '' }}">&lt;</a>
                    
                    @if ($aset_tanah->lastPage() > 1)
                        @php
                            $startPage = max(1, $aset_tanah->currentPage() - 2);
                            $endPage = min($aset_tanah->lastPage(), $aset_tanah->currentPage() + 2);
                            
                            if ($startPage > 1) {
                                echo '<a href="'.route('aset.tanah', [...request()->query(), 'page' => 1]).'" class="page-item">1</a>';
                                if ($startPage > 2) {
                            ...
                            for ($i = $startPage; $i <= $endPage; $i++) {
                                if ($i == $aset_tanah->currentPage()) {
                                    ...
                                    echo '<a href="'.route('aset.tanah', [...request()->query(), 'page' => $i]).'" class="page-item">'.$i.'</a>';
                                }
                            }
                            
                            if ($endPage < $aset_tanah->lastPage()) {
                                if ($endPage < $aset_tanah->lastPage() - 1) {
                                    echo '<span class="dots">...</span>';
                                }
                                echo '<a href="'.route('aset.tanah', [...request()->query(), 'page' => $aset_tanah->lastPage()]).'" class="page-item">'.$aset_tanah->lastPage().'</a>';
                            }
                        @endphp
                    @endif
                    
                    <a href="{{ route('aset.tanah', [...request()->query(), 'page' => $aset_tanah->currentPage()+1]) }}" class="page-item {{ $aset_tanah->currentPage() == $aset_tanah->lastPage() ? 'disabled' : '' }}">&gt;</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function exportTableToExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Aset Tanah");
        
        // Membuat nama file dengan tanggal saat ini
        var now = new Date();
        var fileName = 'Aset_Tanah_' + 
                       now.getFullYear() + 
