...
<div class="form-container">
    ...
    <h2 class="form-title">Tambah Pemanfaatan Kebun Percobaan</h2>
    ...
    <form action="{{ route('lp2tp.pemanfaatan_kp.store') }}" method="POST" enctype="multipart/form-data" id="create-form">
        @csrf
        ...
        <div class="form-group" id="ip2sip-group">
            ...
            <select name="ip2sip_id" id="ip2sip_id" required>
                <option value="" disabled selected>-- Pilih Balai Pengkajian Teknologi Pertanian --</option>
                @foreach($ip2sip as $ip)
                    <option value="{{ $ip->id }}" {{ old('ip2sip_id') == $ip->id ? 'selected' : '' }}>
        ...
        <div class="form-group" id="agro-ekosistem-group">
            ...
            <input type="text" name="agro_ekosistem" id="agro_ekosistem" 
                   value="{{ old('agro_ekosistem') }}" 
                   placeholder="Contoh: Lahan Sawah Irigasi" required>
            <div class="helper-text">
                    ...
                    <table id="diseminasi_table" class="dynamic-table">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <input type="text" 
                                           name="pemanfaatan_diseminasi[0][name]" 
                                           placeholder="Contoh: Demplot Padi Organik" 
                                           value="{{ old('pemanfaatan_diseminasi.0.name') }}" 
                                           class="diseminasi-name"
                                           required>
                                </td>
                                <td>
                                    <input type="number" 
                                           name="pemanfaatan_diseminasi[0][luas]" 
                                           min="0" step="0.001" 
                                           placeholder="Contoh: 2.5" 
                                           value="{{ old('pemanfaatan_diseminasi.0.luas') }}" 
                ...
                <div class="file-upload-container">
                    <div class="file-upload-area" onclick="document.getElementById('dokumentasi').click()">
                        ...
                        <div class="file-upload-content">
                            ...
                            <div class="file-upload-subtext">Klik untuk memilih file atau seret file ke area ini</div>
                        </div>
                    </div>
                </div>
        ...
        <button type="submit" class="submit-button" id="submit-btn">
            <i class="fas fa-save"></i> Simpan Data Pemanfaatan
        </button>
    </form>
</div>
...
<script>
    ...
    let formSubmitted = false; // Flag untuk tracking apakah form sudah di-submit

    // Simplified validation function - only visual border changes after form submission
    const validateForm = () => {
        let isValid = true;
    ...
    const addRowDiseminasi = () => {
        ...
        newRow.innerHTML = `
            <td>
                <input type="text" name="pemanfaatan_diseminasi[${currentIndexDiseminasi}][name]" placeholder="Contoh: Demplot Jagung Hibrida" class="diseminasi-name" required>
            </td>
            <td>
                <input type="number" name="pemanfaatan_diseminasi[${currentIndexDiseminasi}][luas]" min="0" step="0.001" placeholder="Contoh: 1.5" class="diseminasi-luas" required>
            </td>
            <td>
    ...
    const updateUploadAreaText = (fileCount) => {
        ...
        if (fileCount > 0) {
            ...
            uploadSubtext.textContent = 'Klik untuk memilih file atau seret file ke area ini';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Main form fields validation - hanya untuk real-time feedback tanpa error styling
        document.getElementById('ip2sip_id').addEventListener('change', function() {
            if (formSubmitted) validateForm();
        });
        document.getElementById('luas_sip').addEventListener('input', function() {
            if (formSubmitted) validateForm();
        });
        document.getElementById('jumlah_sdm').addEventListener('input', function() {
            if (formSubmitted) validateForm();
        });
        document.getElementById('agro_ekosistem').addEventListener('input', function() {
            if (formSubmitted) validateForm();
        });

        // Table inputs - use event delegation
        document.addEventListener('input', function(e) {
            if ((e.target.classList.contains('diseminasi-name') || e.target.classList.contains('diseminasi-luas')) && formSubmitted) {
                validateForm();
            }
        });
        ...
        document.getElementById('create-form').addEventListener('submit', function(e) {
            formSubmitted = true; // Set flag bahwa form sudah di-submit
            if (!validateForm()) {
                e.preventDefault();
        ...
        // Jangan jalankan initial validation untuk create form
    });
</script>
@endsection