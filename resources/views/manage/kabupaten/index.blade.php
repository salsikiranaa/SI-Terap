...
<table>
    ...
    <tbody>
        @foreach ($kabupaten as $key=>$dt)
            <tr>
                <td class="number">{{ $loop->iteration + ( $kabupaten->currentPage() - 1 ) * $kabupaten->perPage() }}</td>
                <form action="{{ route('manage.kabupaten.update', Crypt::encryptString($dt->id)) }}" method="POST" class="edit">
                    @csrf
                    @method('PUT')
                    <td>
...
<div style="margin: 10px;display:flex;align-items:center;justify-content:center;gap: 5px;">
    <a href="{{ route('manage.kabupaten.view', [...request()->all(), 'page' => 1]) }}" class="paginate-button {{ $kabupaten->currentPage() == 1 ? 'disabled-paginate' : '' }}">First</a>
    <a href="{{ route('manage.kabupaten.view', [...request()->all(), 'page' => $kabupaten->currentPage() - 1]) }}" class="paginate-button {{ $kabupaten->currentPage() == 1 ? 'disabled-paginate' : '' }}"><<</a>
    @for ($i = 1; $i <= $kabupaten->lastPage(); $i++)
        @if ($i > $kabupaten->currentPage() - 5 && $i < $kabupaten->currentPage() + 5)
            <a href="{{ route('manage.kabupaten.view', [...request()->all(), 'page' => $i]) }}" class="paginate-item {{ $kabupaten->currentPage() == $i ? 'active-paginate' : '' }}">{{ $i }}</a>
        @endif
    @endfor
    <a href="{{ route('manage.kabupaten.view', [...request()->all(), 'page' => $kabupaten->currentPage() + 1]) }}" class="paginate-button {{ $kabupaten->currentPage() == $kabupaten->lastPage() ? 'disabled-paginate' : '' }}">>></a>
    <a href="{{ route('manage.kabupaten.view', [...request()->all(), 'page' => $kabupaten->lastPage()]) }}" class="paginate-button {{ $kabupaten->currentPage() == $kabupaten->lastPage() ? 'disabled-paginate' : '' }}">Last</a>
</div>
{{--  end pagination --}}
